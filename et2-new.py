...
while Q:
    ...
    if len(solution) == width+int(sys.argv[2]):
        if True or solcount % 100000 == 0:
            counts = Counter([pieces[p][1] for p in solution[width:-1]])
            countstr = f"{int(''.join(map(str, sorted(counts.values())))):04d}"[::-1]
            disp = ''.join([f'{p[0]:02d}' for p in solution[width:]])
            print(f'{solcount} {countstr} {disp}')
        solcount += 1
    else:
